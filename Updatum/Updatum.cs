...
public partial class Updatum : INotifyPropertyChanged
{
    ...
    private string _assetRegexPattern = RuntimeInformation.RuntimeIdentifier;
    ...
    /// </summary>
    /// <remarks>Default to <see cref="RuntimeInformation.RuntimeIdentifier"/>.</remarks>
            ...
            return Task.Run(() =>
            {
                ...
                if (fileExtension.Equals(".zip", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    else // Portable app
                    {
                        ...
                        var currentExecutablePath = EntryApplication.IsDotNetSingleFileApp
                            ? EntryApplication.AssemblyLocation
                            : EntryApplication.ExecutablePath;
                        ...
                        if (OperatingSystem.IsWindows())
                        {
                            ...
                            using (var stream = File.CreateText(upgradeScriptFilePath))
                            {
                                ...
                                if (killCommands.Count > 0)
                                {
                                    ...
                                    stream.WriteLine(" :: IM - Image name (process filename)");
                                    stream.WriteLine(" :: /F - Forceful termination (without this, it tries graceful first)");
                                    stream.WriteLine(" :: /T - Terminate child processes");
                                    stream.WriteLine("timeout /t 1");
                                    stream.WriteLine();
                                }

                                stream.WriteLine("echo - Sync/copying files over");
                                stream.WriteLine("robocopy >nul 2>&1");
                                stream.WriteLine("if % errorlevel % lss 16(");
                                stream.WriteLine("  robocopy \"% SOURCE_PATH%\" \"%DEST_PATH%\" /E /COPY:DAT /ZB /R:3 /W:3 /MT:4");
                                stream.WriteLine("   :: /E - Copies all subfolders, including empty ones.");
                                stream.WriteLine("   :: /MIR - Mirror (sync) source and destination (deletes extra files in destination).");
                                stream.WriteLine("   :: /COPY:DAT - Copies data, attributes, and timestamps (no admin needed)..");
                                stream.WriteLine("   :: /ZB - Uses restartable mode (better for large files).");
                                stream.WriteLine("   :: /MOVE - Move files and directories (delete from source after copying)");
                                stream.WriteLine("   :: /R:n - Retries on failed copies.");
                                stream.WriteLine("   :: /W:n - Wait time between retries in seconds.");
                                stream.WriteLine("   :: /MT:n - Multithreaded copies (faster).");
                                stream.WriteLine(") else (");
                                stream.WriteLine("  xcopy \"%SOURCE_PATH%\\*\" \"%DEST_PATH%\" /E /H /Y /C");
                                stream.WriteLine("   :: /E - Copies all subdirectories, including empty ones.");
                                stream.WriteLine("   :: /H - Copies hidden and system files.");
                                stream.WriteLine("   :: /Y - Suppresses prompting to overwrite files.");
                                stream.WriteLine("   :: /C - Continues copying even if errors occur.");
                                stream.WriteLine(")");
                                stream.WriteLine();
                                ...
                                if (!string.IsNullOrWhiteSpace(newExecutingFilePath) && runArguments != NoRunAfterUpgradeToken)
                                {
                                    ...
                                    stream.WriteLine(EntryApplication.IsRunningFromDotNetProcess
                                        ? $"  start \"\" \"{EntryApplication.ExecutablePath}\" \"{newExecutingFilePath}\" {runArguments}"
                                ...
                                stream.WriteLine("echo - Removing temp source folder");
                                stream.WriteLine("rmdir /S /Q \"%SOURCE_PATH%\"");
                                stream.WriteLine();

#if !DEBUG
                                stream.WriteLine("echo - Removing self");
                                stream.WriteLine($"del /F /Q \"{upgradeScriptFilePath}\"");
#endif
                            ...
                            using (var stream = File.CreateText(upgradeScriptFilePath))
                            {
                                ...
                                if (!string.IsNullOrWhiteSpace(newExecutingFilePath) && runArguments != NoRunAfterUpgradeToken)
                                {
                                    ...
                                    if (EntryApplication.IsRunningFromDotNetProcess)
                                    {
                                        ...
                                        stream.WriteLine($"  nohup \"{EntryApplication.ExecutablePath}\" \"{newExecutingFilePath}\" {runArguments} >/dev/null 2>&1 &");
                                    }
                                    else
                                    {
                                    ...
                                    stream.WriteLine("  echo \"- Detached process (PID: $!)\"");
                ...
                if (fileExtension == string.Empty
                    ...
                    || (fileExtension.Equals(".exe", StringComparison.OrdinalIgnoreCase) &&
                        EntryApplication.IsSingleFileApp))
                {
